...
export default function TakeawayBox({
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`relative overflow-hidden rounded-xl bg-gradient-to-br from-gold-500/10 via-gold-500/5 to-transparent border border-gold-500/20 ${className}`}
    >
      {/* Decoração de fundo */}
      ...
      <div className={variant === "compact" ? "p-4 sm:p-5" : "p-6 sm:p-8"}>
        ...
        <ul className="space-y-3">
          {points.map((point, index) => (
            ...
            >
              ...
              <span className={`text-navy-200 ${variant === "compact" ? "text-sm" : "text-base"}`}>
                {point}
              </span>
            </motion.li>
          ))}
        ...
        {cta && (
          <motion.a
            href={cta.href}
            whileHover={{ x: 4 }}
            className="inline-flex items-center gap-2 mt-6 text-gold-400 hover:text-gold-300 font-medium transition-colors"
          >
            {cta.text}
