      ...
      <div className="relative pl-8 border-l-2 border-green-500/20 space-y-6">
        {items.map((item, index) => (
          <div key={index} className="relative">
            {/* Dot */}
            <div className="absolute -left-[25px] p-1 bg-slate-50 dark:bg-navy-950 rounded-full">
              {item.completed ? (
            ...
            <div className="pb-2">
              ...
              {item.description && (
                <p className="text-sm text-slate-500 dark:text-navy-400 mt-1">{item.description}</p>
              )}
            </div>
          </div>
        ))}
