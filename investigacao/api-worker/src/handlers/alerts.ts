/**
 * Alerts Handlers - API Routes para Alertas
 */
...
export async function handleGetAlertConfig(
  ...
  ).bind(user.uid).first<{ tenant_id: string }>();
  ...
  ).bind(userRecord.tenant_id).first();
...
export async function handleUpdateAlertConfig(
  ...
  if (existing) {
    ...
    await env.DB.prepare(
      ...
      body.notification_email !== undefined ? body.notification_email : null,
      now,
